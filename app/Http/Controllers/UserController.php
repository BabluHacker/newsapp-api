...
class UserController extends Controller
{
    ...
    {
        $this->middleware('auth', ['except' => ['createOrUpdate', 'auth']]);
    }
    ...
    public function auth(Request $request)
    {
        ...
        $accessTokenModel = AccessToken::where('access_token', 'like', $request->input('token'))->first();
        if($accessTokenModel){
            ...
            if(isset($userAccount['error'])) {
                ...
                if($userModel){ //new login, old user
                    $res = AccessToken::updateAccessToken($userModel->id, $request->input('token'));
                    if($res){
                        $response = [
                            'status' => 1,
                            'data'  => $userModel
    ...
    public function createOrUpdate(Request $request)
    {
        ...
        if(isset($userAccount['error'])){
            ...
            if($modelUser){
                ...
                $modelAT = AccessToken::updateOrCreate(['user_id'=>$modelUser->id], ['access_token'=>$token]);
                $response = [
                    'status' => 1,
                    'data'  => $modelUser,
                    'at' => $modelAT
    ...
    public function me(Request $request){
        ...
        $modelAT = AccessToken::where('access_token', 'like', $headers['x-access-token'][0])->first();
